...
assert.match(main, /!state\.characterTurn \|\| state\.gameLost/);
assert.match(main, /if \(next !== state\) setState\(next\)/);
assert.match(main, /advanceCharacterTurn/);
assert.match(main, /playSimpleAction/);
assert.match(main, /stage-zone-arrow/);
assert.match(main, /weather-audio/);
assert.match(main, /renderStoryCast/);
assert.match(main, /generateActorDialogue/);
assert.doesNotMatch(main, /predicted-positive/);
...
assert.doesNotMatch(main.slice(main.indexOf("function renderGame"), main.indexOf("function nextMoveCopy")), /resources|forecast|journal/);
...
assert.match(styles, /\.story-cast/);
assert.match(styles, /\.actor-dialogue/);
